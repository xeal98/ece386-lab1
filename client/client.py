...
def get_img_prediction(
    ...
    postInfo = requests.post(f"http://{server_ip}:{server_port}{api_path}", headers=headers, data=bytes(file.read()), timeout=2)
...
def main(server_ip: str, server_port: int) -> None:
    ...
    while(True):
        ...
        serverPrediction = get_img_prediction(server_ip, server_port, "/predict", filePath)
